...
class Schedule_table extends CI_Controller
{
  ...
  public function index()
  {
    ...
    if ($result_role[2]['subMenu1'][0]['subMenu2'][2]['subMenu3'][1]['isAllow'] != true) {
    ...
    if (isset($schedules['data'])) {
      foreach ($schedules['data'] as $schedule) {
        if ($schedule['propTaskMaintenance']) {
          foreach ($schedule['propTaskMaintenance'] as $key2 => $propTaskMaintenance) {

            // if (isset($propTaskMaintenance['propItemProgress']['progressStatus'])) {
            //   $propProgress = $propTaskMaintenance['propItemProgress']['progressStatus'];
            // } else {
            ...
            $newSchedules[] = [
              ...
              "idAsset" => $propTaskMaintenance['propAsset']['idAsset'],
              "assetName" => $propTaskMaintenance['propAsset']['assetName'],
              "assetCode" => $propTaskMaintenance['propAsset']['assetCode'],
              "roomName" => $propTaskMaintenance['propAsset']['propAssetPropadmin']['propAssetPropbuildingRoom']['roomName'],
    ...
    $this->load->view('task/electromedic_task/maintenance_schedule_med_table', $data);
