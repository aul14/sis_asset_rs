...
class Schedule_report extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($result_role[2]['subMenu1'][0]['subMenu2'][1]['subMenu3'][0]['isAllow'] != true) {
            exit('You dont have access!!');
        }

        $this->load->view('components/header');
        $this->load->view('components/sidebar', $data);
        $this->load->view('task/electromedic_task/print/inspection/task_print');
        $this->load->view('task/electromedic_task/form/inspection_form');
        $this->load->view('task/electromedic_task/form/inspection_form_report');
        $this->load->view('task/electromedic_task/form/inspection_form_times');
        $this->load->view('task/electromedic_task/inspection_schedule_med_report');
    ...
    private function task_file_upload($file, $docNumber = NULL)
    {
        ...
        $fileCat = $this->m_file_cat->fileCatByFileCatName('INSPFile');
    ...
    public function task_by_id()
    {
        ...
        foreach ($idTask as $key => $id) {
            ...
            $idAssetMaster = $task['propTaskInspection'][0]['propAsset']['idAssetMaster'];

            $task['formTemplate'] = $this->m_form_template->formTemplateByTypeMaster($idAssetMaster, 1)['data'];
        }
    ...
    public function store_form_edit()
    {
        ...
        $task['data']['propTaskInspection'][0]['noteInspection'] = $post['maintenanceNote'];
        $task['data']['propTaskInspection'][0]['inspectionResult'] = !empty($post['finalResult']) ? $post['finalResult'] : "";
        $task['data']['propTaskInspection'][0]['idForm'] = (int)$insert['data'];
        $task['data']['propTaskInspection'][0]['propForm'] = $get_form['data'];
    ...
    public function store_form_old_edit()
    {
        ...
        $task['data']['propTaskInspection'][0]['noteInspection'] = $post['maintenanceNote'];
        $task['data']['propTaskInspection'][0]['inspectionResult'] = !empty($post['finalResult']) ? $post['finalResult'] : "";
        $task['data']['propTaskInspection'][0]['idForm'] = (int)$post['idForm'];
        $task['data']['propTaskInspection'][0]['propForm'] = $get_form['data'];
        $task['data']['propTaskMaterial'] = !empty($propTaskMaterial) ? $propTaskMaterial : [];
        $task['data']['propTaskFiles'] = $task_files_insert;
    ...
    public function store_form_schedule_edit()
    {
        ...
        $task['data']['propTaskInspection'][0]['idFormTemplate'] = $post['formTemplateEdit'];
    ...
    public function store()
    {
        ...
        $schedule = $this->m_schedule->data(
            $post['idSchedule'] ? $post['idSchedule'] : 0,
            $parentSchedule = 0,
            $scheduleType = $post['scheduleType'], //$post['scheduleType'],
            $scheduleName = 'INSPECTION',
            $scheduleDesc = 'INSPECTION',
            $post['scheduleStart'],
        ...
        $progress = $this->m_progress->data(
            ...
            $idPendingBy = 0,
            $idApproveBy = 0,
            $idAssignee = isset($post['idAssignee']) ? $post['idAssignee'] : $this->session->userdata('id_user'),
            $idDelegator = 0,
            $initBy = '',
            $responBy = '',
            $startBy = '',
            ...
            $approveBy = '',
            $delegateBy = '',
            $assignTo = isset($post['assignTo']) ? $post['assignTo'] : $this->session->userdata('username')
        );
        ...
        if ($post['formType'] == 'add') {
            ...
            foreach ($post['idAsset'] as $key => $value) {
                ...
                $taskInspection[] = $this->m_task_maintenance->data(
                    $post['idTask'] ? $post['idTask'] : 0,
                    $post['idAsset'][$key],
                    $isNeedPart = 0,
                ...
                $task['propTaskInspection'] = $taskInspection;
    ...
    public function report()
    {
        ...
        if ($button_pdf == 'pdf') {
            ...
            $data['mpdf'] = $mpdf;
            $html = $this->load->view('task/electromedic_task/print/inspection/pdf', $data, TRUE);
            $mpdf->WriteHTML($html);
            ...
            $this->load->view('task/electromedic_task/print/inspection/excell', $data);
        }
    }
    ...
    public function data_table()
    {
        ...
        if (sizeof($posts) > 0) {
            ...
            foreach ($posts['data'] as $key => $value) {
                ...
                // $posts['data'][$key] = [];
                foreach ($value['propTaskInspection'] as $propTaskInspection) {
                    $idAsset = $propTaskInspection['idAsset'];
                    $idTask = $propTaskInspection['idTask'];
                    $idFormTemplate = $propTaskInspection['idFormTemplate'];
                    ...
                    $posts['data'][$key]['inspectionResult'] = $propTaskInspection['inspectionResult'];
                    ...
                    title="Finish Not Approved" class="tip">';
                        ...
                        $posts['data'][$key]['assetCode']      =  $propTaskInspection['propAsset']['assetCode'];
                        $posts['data'][$key]['scheduleStart']      =  $scheduleStart1;
                        $posts['data'][$key]['assetName']      =  $propTaskInspection['propAsset']['assetName'];
                        $posts['data'][$key]['merk']      =  $propTaskInspection['propAsset']['propAssetPropgenit']['merk'];
                        $posts['data'][$key]['tipe']      =  $propTaskInspection['propAsset']['propAssetPropgenit']['tipe'];
                        $posts['data'][$key]['serialNumber']      =  $propTaskInspection['propAsset']['propAssetPropgenit']['serialNumber'];
                        $posts['data'][$key]['roomName']      =  $propTaskInspection['propAsset']['propAssetPropadmin']['propAssetPropbuildingRoom']['roomName'];
                        $posts['data'][$key]['floorName']      =  $propTaskInspection['propAsset']['propAssetPropadmin']['propAssetPropbuildingRoom']['floorName'];
                        $posts['data'][$key]['buildingName']      =  $propTaskInspection['propAsset']['propAssetPropadmin']['propAssetPropbuildingRoom']['buildingName'];
    ...
    public function update_inp()
    {
        ...
        $formTemplate = $this->m_form_template->formTemplateById($task[0]['propTaskInspection'][0]['idFormTemplate'])['data'];
