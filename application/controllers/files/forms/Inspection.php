...
class Inspection extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($result_role[3]['subMenu1'][0]['subMenu2'][0]['isAllow'] != true) {
            exit('You dont have access!!');
        }

        $this->load->view('components/header');
        $this->load->view('components/sidebar', $data);
        $this->load->view('files/forms/form/inspection_form');
        $this->load->view('files/forms/form_inspection_index');
    ...
    public function store()
    {
        ...
        $propFormTemplateitem = [
            "idTmpItem" => !empty($post['idTmpItem']) ? (int)$post['idTmpItem'] : 0,
            "idTemplate" => !empty($post['idFormTemplate']) ? (int)$post['idFormTemplate'] : 0,
            "idForm" => !empty($post['idForm']) ? (int)$post['idForm'] : 0,
            "tmpitemEQ" => false,
