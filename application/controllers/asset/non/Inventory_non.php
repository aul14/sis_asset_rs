...
class Inventory_non extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($result_role[1]['subMenu1'][1]['subMenu2'][0]['isAllow'] != true) {
            exit('You dont have access!!');
        }

        $this->load->view('components/header');
        $this->load->view('components/sidebar', $data);
        $this->load->view('assets/non_medical_equipment/inventory_non_index', $data);
        $this->load->view('assets/non_medical_equipment/form/inventory_form');
    ...
    public function store()
    {
        ...
        if (!empty($input['idAsset'])) {
            ...
            // redirect('asset/non/inventory_non', 'refresh');
    ...
    public function report()
    {
        ...
        if ($button_pdf == 'pdf') {
            if ($jenis == "summary") {
                ...
                $data['asset_masters'] = $this->m_asset->report_non_asset_condition();
                ...
                $data['mpdf'] = $mpdf;
                $html = $this->load->view('assets/non_medical_equipment/print/summary_pdf', $data, TRUE);
                $mpdf->WriteHTML($html);
                $mpdf->Output();
            } else {
            ...
            if ($jenis == "summary") {
                ...
                $this->load->view('assets/non_medical_equipment/print/summary_excell', $data);
            } else {
                ///////// ADVANCED SEACRH DATE ///////////////////
                if ($startDateq3 != '') {
            ...
            foreach (html_escape($this->input->post()['assetList']) as $field) {
                echo " 
                    <table style='font-size: 13px;'>
                        <tr>
                            <td><img src='" . (new QRCode)->render($field['assetCode']) . "' alt='QR Code' width='80'/>'</td>
                            <td>
                                <span style='display: block; font-weight: bold;'>" . $this->session->userdata('hospital') . "</span>
                                <span style='display: block;'>" . $field['assetName'] .  "</span>
                                <span style='display: block;'>" . $field['propAssetPropgenit']['merk'] . " / " . $field['propAssetPropgenit']['tipe'] . "</span>
                                <span style='display: block;'>S/N: " . $field['assetCode'] . "</span>
                            </td>
                        </tr>
                    </table>
                ";
