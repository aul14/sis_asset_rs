...
class Inventory_me extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $vmode = $this->input->get('vmode') ? $this->input->get('vmode') : 'MEQ';
        ...
        switch ($vmode) {
            case "Inventory":
                $data['columns'] = [
                    'Code',
                    // 'Simak Code',
                    'Label Code', 'Asset Name', 'Merk', 'Type', 'SN', 'Status', 'Room Name', 'Floor Name', 'Building', 'Year Procurement',
                    'Condition', 'Vendor/Company', 'Last Calibrated', 'Next Calibrated', 'CAL. Required', 'Ownership Type', 'Risk Level'
                ];
                ...
                $data['rows'] = [
                    ...
                    'propAssetPropgenit.serialNumber', 'propAssetPropadmin.status', 'propAssetPropadmin.propAssetPropbuildingRoom.roomName',
                    'propAssetPropadmin.propAssetPropbuildingRoom.floorName',
                    'propAssetPropadmin.propAssetPropbuildingRoom.buildingName', 'procureDate',
                    'propAssetPropadmin.condition', 'propAssetPropadmin.propContact.contactCompany', 'propAssetPropmedeq.lastCalib', 'propAssetPropmedeq.nextCalib',
                    'calibMust', 'propAssetPropadmin.ownershipType', 'propAssetPropadmin.riskLevel'
                ];
        ...
        if ($result_role[1]['subMenu1'][0]['subMenu2'][0]['isAllow'] != true) {
            exit('You dont have access!!');
        }

        $this->load->view('components/header');
        $this->load->view('components/sidebar', $data);
        $this->load->view('assets/medical_equipment/inventory_med_index', $data);
        $this->load->view('assets/medical_equipment/form/inventory_form');
    ...
    public function report()
    {
        ...
        if ($button_pdf == 'pdf') {
            ...
            if ($jenis == "summary") {
                ...
                $data['asset_masters'] = $this->m_asset->report_med_asset_condition();
                ...
                $data['mpdf'] = $mpdf;
                $html = $this->load->view('assets/medical_equipment/print/summary_pdf', $data, TRUE);
                $mpdf->WriteHTML($html);
                $mpdf->Output();
            } else {
                ...
                $mpdf = new \Mpdf\Mpdf($mpdfConfig);
                $data['mpdf'] = $mpdf;
                $html = $this->load->view('assets/medical_equipment/print/pdf_inv', $data, TRUE);
                $mpdf->WriteHTML($html);
                $mpdf->Output();
            }
        } else {
            ...
            if ($jenis == "summary") {
                ...
                $this->load->view('assets/medical_equipment/print/summary_excell', $data);
            } else {
                ...
                $this->load->view('assets/medical_equipment/print/excell_inv', $data);
            }
        }
    }
    ...
    public function asset_by_id()
    {
        ...
        foreach ($id_asset as $key => $id) {
            $result = $this->m_asset->assetById($id);
        }
        ...
        if (!empty($result['data']['parentAssetID'])) {
            $result_parent = $this->m_asset->assetById($result['data']['parentAssetID']);
            $show['data_parent'] = $result_parent['data'];
        }
    ...
    public function detail_asset_by_id()
    {
        $show = [];
        if (!$this->input->is_ajax_request()) {
            exit('No direct script access allowed');
        ...
        foreach ($id_asset as $key => $id) {
            $result = $this->m_asset->assetById($id);
            $task = $this->m_asset->assetTaskSimple($id);
        }

        // buat data by id
        // $result2 = $this->m_asset_master->assetMasterList();
        $show['data_task'] = $task['data'];
        $show['data_detail'] = $result['data'];


        if (!empty($result['data']['propAssetPropfiles'])) {
            $id_file = [];
            foreach ($result['data']['propAssetPropfiles'] as $key3 => $val) {
                $id_file[] = $val['idFile'];
            }
            foreach ($id_file as $key4 => $id2) {
                $view_file[] = $this->m_file->fileBase64($id2)['data'];
            }

            if (!empty($view_file)) {
                $show['view_file'] = $view_file;
            } else {
                $show['view_file'] = "";
            }
        }
    ...
    public function print_qr_new()
    {
        ...
        if (isset($this->input->post()['assetList'])) {
            $asset_list = [];
            foreach (html_escape($this->input->post()['assetList']) as $field) {
                $asset_list[] = $field;
            }

            $data['data_list'] = $asset_list;

            // echo json_encode($data);
            // die;
            $this->load->view('assets/qr/print_qr_new', $data);
        }
    }
    ...
    public function print_qr()
    {
        ...
        if (isset($this->input->post()['assetList'])) {
            ...
            foreach (html_escape($this->input->post()['assetList']) as $field) {
                ...
                <body>
                    <table style='width:70mm;margin:2.5mm 1mm 2mm 1mm;page-break-after: always'>
                        <tbody>
                            <tr style='position:relative;'>
                                <td colspan='3'>
                                    <h1 class='title' style='text-align:center; font-size:10px; font-weight:bold; color: #000000; margin:0 0 0 0px; '>
                                        <div>" . $this->session->userdata('hospital') . "</div>
                            ...
                            <tr>
                                <td style=''>
                                    <img src='" . (new QRCode)->render($field['assetCode']) . "' alt='QR Code' style='display:block;margin:0;' width='65'>
                                </td>
                                <td style='padding:0px 3px 0px 3px; width:100%'>
                                    <table class='tl' style='font-size:10px;width:100%;color: #000000; margin:10 10 10 10px;'>
                                        <tbody>
                                            <tr>
                                                <td style='font-size:10px;'>" . $field['assetName'] . "</td>
                                            </tr>
                                            <tr>
                                                <td style='font-size:10px;'>" . $field['propAssetPropgenit']['merk'] . "</td>
                                               
                                            </tr>
                                            <tr>
                                                <td style='width:100%;padding-right: 5px;font-size:10px;'>
                                                    SN : " . $field['propAssetPropgenit']['serialNumber'] . "
                                                </td>
                                                
                                            </tr>
                                            <tr>
                                                <td style='font-size:10px;'>" . $field['propAssetPropadmin']['propAssetPropbuildingRoom']['roomName'] . "</td>
                                                <td>
                                                    <div style='font-size:10px; font-weight: bold; margin-left: -80px; float: right;'>" . $field['assetCode'] . "</div>
                                                </td>
                                            </tr>
            ...
            $field = html_escape($this->input->post());
    ...
    public function print_qr_kecil()
    {
        if (!$this->input->is_ajax_request()) {
            exit('No direct script access allowed');
        }
        ...
        if (isset($this->input->post()['assetList'])) {
            ...
            foreach ($this->input->post()['assetList'] as $field) {
                echo "
                <!DOCTYPE html>
                <html lang='en'>
