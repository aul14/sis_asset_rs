...
class Master_ecri extends CI_Controller
{
  ...
  public function index()
  {
    ...
    // $this->load->view('master_data/Master_ecri/form/Master_ecri_form');
    $this->load->view('master_data/master_ecri/master_ecri_index');
  ...
  public function data_table()
  {
    ...
    $count = $this->m_master_ecri->ecriQuery(
      [
    ...
    $posts = $this->m_master_ecri->ecriQuery($request);

    if (sizeof($posts) > 0) {
      $no = $start + 1;
      foreach ($posts['data'] as $key => $value) {
        $ecriCode = $value['ecriCode'];

        $posts['data'][$key]['check_box_cuk'] = "<input type='checkbox' id='delcheck_{$ecriCode}'  name='msg[]' class='delete_check' value='{$ecriCode}' />";
  ...
  public function query()
  {
    ...
    $result = $this->m_master_ecri->ecriQuery($request);
