<div id="bld_form" class="modal samrs-modal zoom fade" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-xl">
    <form id="bld-form">
      <div class="modal-content">
        <div class="modal-header">
          <p id="title-asset">Add New Building</p>
        ...
        <div class="modal-body fixed-height samrs-form" id="app_form">
          ...
          <main-form>
            <template v-slot:kategori-tab>
              ...
              <input name="subSysCat" id="subSysCat" type="hidden" value="BLD">
        ...
        <div class="modal-footer">
          ...
          <a class="btn samrs-danger is-outline" href="<?= base_url('asset/building/inventory_bld'); ?>">Cancel</a>
        </div>
      </div>
    </form>
  </div>
...
<script>
  $(function() {
    ...
    // proses save
    $('#bld-form').submit(function(e) {
      e.preventDefault();
      $.ajax({
        url: BASE_URL + 'asset/building/inventory_bld/store',
        type: "POST",
          ...
          if (response.queryResult == true) {
            if (!is_close) {
              ...
              Swal.fire({
                ...
                $('.samrs-table1').DataTable().ajax.reload();
                document.location.href = "<?php echo base_url(); ?>asset/building/inventory_bld";
              });
  ...
  function onChangeMasterAsset() {
    $('.with-ajax-masterAsset').change(function(e) {
      ...
      if (idAssetMaster != undefined) {
        $.ajax({
          type: "post",
          url: BASE_URL + "asset/me/inventory_me/ajax_asset_master_by_id",
          data: {
  ...
  function calcDepre1() {
    ...
    $.ajax({
      type: 'post',
      url: BASE_URL + 'asset/me/inventory_me/calc_depre',
      data: values,
  ...
  function evtSelectFileCat(evt) {
    var options = {
      ajax: {
        ...
        emptyRequest: true,
        url: BASE_URL + "asset/me/inventory_me/ajax_file_cat",
        type: 'POST',
        dataType: 'json',
  ...
  function evtMasterAssetInstrument(evt) {
    var options = {
      ajax: {
        // url     : SITE_URL + "assets/vendors/bootstrap-select-ajax/example/php/ajax.php",
        emptyRequest: true,
        url: BASE_URL + "asset/me/inventory_me/ajax_asset_master_instrument",
        type: 'POST',
        dataType: 'json',
        // Use "{{{q}}}" as a placeholder and Ajax Bootstrap Select will
        // automatically replace it with the value of the search query.
