<main dir="ltr" class="page-wrapper full-height" id="App">
  <main-app>
    ...
    <action-button-card title="Building - Inventory">
      <template v-slot:buttons>
        <table-init></table-init>
        <?php if ($result_role[1]['subMenu1'][2]['subMenu2'][0]['subMenu3'][0]['isAllow'] == true) : ?>
          <add-data modal="bld_form"></add-data>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][2]['subMenu2'][0]['subMenu3'][1]['isAllow'] == true) : ?>
          <edit-data button-id="modal_edit_inventory"></edit-data>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][2]['subMenu2'][0]['subMenu3'][2]['isAllow'] == true) : ?>
          <delete-data button-id="modal_delete_inventory"></delete-data>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][2]['subMenu2'][0]['subMenu3'][3]['isAllow'] == true) : ?>
          <detail-data button-id="modal_detail_inventory"></detail-data>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][2]['subMenu2'][0]['subMenu3'][4]['isAllow'] == true) : ?>
          <print-data modal="assets_print"></print-data>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][2]['subMenu2'][0]['subMenu3'][5]['isAllow'] == true) : ?>
          <table-advance-search overlay="advanced_search"></table-advance-search>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][2]['subMenu2'][0]['subMenu3'][6]['isAllow'] == true) : ?>
          <table-quick-view></table-quick-view>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][2]['subMenu2'][0]['subMenu3'][7]['isAllow'] == true) : ?>
          <table-column-view></table-column-view>
        <?php endif; ?>
      </template>
      <template v-slot:content>
        <form action="<?= base_url('asset/building/inventory_bld'); ?>" method="get">
    ...
    <table-view-card>
      <template v-slot:table-content>
        ...
        <table class="samrs-table1 table samrs-tableview samrs-table-striped table-hover w-100">
          <thead>
            <tr>
              <th class="text-center" style="width:50px !important">
...
<script>
  function TableData() {
    var table = $('.samrs-table1').DataTable({
      ...
      "ajax": {
        "url": BASE_URL + "asset/building/bld_datatable/bld_data_table?vmode=" + "<?= $this->input->get('vmode'); ?>",
        "dataType": "json",
        "data": {
  ...
  function qr_code(table) {
    $('#modal_qr_inventory').click(function() {
      // e.preventDefault();
      var idAsset = $('input:checkbox[class=delete_check]:checked');
      // Read all checked checkboxes
      // console.log(idAsset);
      if (idAsset.length == 1) {
        var itemData = table.rows({
          selected: true
        }).data()[0];
        // console.log(itemData);
        $.ajax({
          type: "POST",
          url: BASE_URL + "asset/me/inventory_me/print_qr",
          data: itemData,
          // dataType: "json",
            ...
            if (res.data_detail.propChildAsset != null) {
              ...
              if (res.data_detail.propChildAsset.length > 0) {
                for (i = 0; i < res.data_detail.propChildAsset.length; i++) {
                  ...
                  html += '<td>' + res.data_child_supplier.propContact.contactCompany + '</td>';
            ...
            if (res.data_detail.propAssetPropfiles != null) {
              ...
              if (res.data_detail.propAssetPropfiles.length > 0) {
                for (i = 0; i < res.data_detail.propAssetPropfiles.length; i++) {
                  ...
                  html2 += '<td>' + res.data_detail.propAssetPropfiles[i].propFileName + '</td>';
                  html2 += '<td>' + res.data_detail.propAssetPropfiles[i].propFile.fileSize + '</td>';
                  html2 += '<td>' + '<a href="<?= base_url(); ?>file/file_download/' +
                    res.data_detail.propAssetPropfiles[i].propFile.idFile + '"><i class"mdi mdi-download"></i>' + res.data_detail.propAssetPropfiles[i].propFileName + '</a>' + '</td>';
                  html2 += "</tr>";
                }
              }
            } else {
              html2 = '<tr><td colspan="4" class="text-center">Data Kosong</td></tr>';
            }
  ...
  function edit() {
    ...
    $('#modal_edit_inventory').click(function() {
      ...
      if (idAsset.length == 1) {
        $('#bld_form').find('.samrs-form').append(`
          <div class="loader">
            <div></div>
            <div></div>
          </div>
        ...
        $.ajax({
          ...
          dataType: "json",
          success: function(res) {
            $('#title-asset').html('Updated New Building');
            ...
            $('input[name=propAssetPropbuildingFloor_idBuilding]').val(res.data_update.idAsset);
            ...
            if (res.data_update.propAssetProptax != null) {
            ...
            if (res.data_update.propAssetProptaxother != null) {
            ...
            if (res.data_update.propAssetPropbuilding.propAssetPropbuildingFloor.length > 0) {
              for (i = 1; i < res.data_update.propAssetPropbuilding.propAssetPropbuildingFloor.length; i++) {
                $(".input-hidden-bld").append(`<input type="hidden" name="propAssetPropbuildingFloor_idFloor[]" value="` + res.data_update.propAssetPropbuilding.propAssetPropbuildingFloor[i].idFloor + `">`);
                // $(".input-hidden-bld").append(`<input type="hidden" name="propAssetPropbuildingFloor_idBuilding" value="` + res.data_update.propAssetPropbuilding.propAssetPropbuildingFloor[i].idBuilding + `">`);
                $(".floor-first").val(res.data_update.propAssetPropbuilding.propAssetPropbuildingFloor[0].floorName);
                $(".id-floor-first").val(res.data_update.propAssetPropbuilding.propAssetPropbuildingFloor[0].idFloor);
                $(".id-building-first").val(res.data_update.propAssetPropbuilding.propAssetPropbuildingFloor[0].idBuilding);
                var addfield =
                  '<div class="floor-form">' + $(`<div class="floor-formcopy">
                  <div class="input-group mt-10 mb-10">
                    <input type="text" class="form-control" name="propAssetPropbuildingFloor_floorName[]" value="` + res.data_update.propAssetPropbuilding.propAssetPropbuildingFloor[i].floorName + `">
                    <div class="input-group-append">
                      <button class="btn samrs-danger remove-floor"
                      type="button" name="button"
                      title="Remove Floor"><i class="fas fa-times"></i></button>
                    </div>
                  </div>
                </div>`).html() + "</div>";
                $("body").find(".floor-form:last").after(addfield);
              }
            }
            $('#bld_form').find('.loader').remove();
          }
        });
      } else {
        Swal.fire({
