<main dir="ltr" class="page-wrapper full-height" id="App">
  <main-app>
    ...
    <action-button-card title="Medical Equipment - Inventory">
      <template v-slot:buttons>
        <table-init></table-init>
        <?php if ($result_role[1]['subMenu1'][0]['subMenu2'][0]['subMenu3'][0]['isAllow'] == true) : ?>
          <add-data modal="inventory_form"></add-data>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][0]['subMenu2'][0]['subMenu3'][1]['isAllow'] == true) : ?>
          <edit-data button-id="modal_edit_inventory"></edit-data>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][0]['subMenu2'][0]['subMenu3'][2]['isAllow'] == true) : ?>
          <delete-data button-id="modal_delete_inventory"></delete-data>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][0]['subMenu2'][0]['subMenu3'][3]['isAllow'] == true) : ?>
          <detail-data button-id="modal_detail_inventory"></detail-data>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][0]['subMenu2'][0]['subMenu3'][4]['isAllow'] == true) : ?>
          <print-qr-data button-id="modal_qr_inventory"></print-qr-data>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][0]['subMenu2'][0]['subMenu3'][5]['isAllow'] == true) : ?>
          <print-data modal="assets_print"></print-data>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][0]['subMenu2'][0]['subMenu3'][6]['isAllow'] == true) : ?>
          <table-advance-search overlay="advanced_search"></table-advance-search>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][0]['subMenu2'][0]['subMenu3'][7]['isAllow'] == true) : ?>
          <table-quick-view>
          </table-quick-view>
        <?php endif; ?>
        <?php if ($result_role[1]['subMenu1'][0]['subMenu2'][0]['subMenu3'][8]['isAllow'] == true) : ?>
          <table-column-view></table-column-view>
        <?php endif; ?>
      </template>
      <template v-slot:content>
        <form action="<?= base_url('asset/me/inventory_me'); ?>" method="get">
      ...
      if (idAsset.length == 1) {
        ...
        $.ajax({
          ...
          success: function(res) {
            ...
            if (res.data_update.propAssetPropaspak.insCode != "") {
              $('#option-ajax-roomaspak').html(res.data_update.propAssetPropaspak.insCode);
              $('#option-ajax-roomaspak').val(res.data_update.propAssetPropaspak.insCode);
              $('.selectpicker-roomaspak').selectpicker('refresh');
            }
