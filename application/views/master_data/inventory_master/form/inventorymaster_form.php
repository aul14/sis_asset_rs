<div id="inventorymaster_form" class="modal samrs-modal zoom fade" data-backdrop="static" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <form action="<?= base_url('master_data/inventory_master/store'); ?>" method="post" id="addform">
            <div class="modal-content">
                <div class="modal-header">
                    <p id="title-master">add new inventory master</p>
                ...
                <div class="modal-body fixed-height samrs-form" id="app_form">
                    <form-inventorymaster>
                        <template v-slot:catcode-slot>
                            <select name="catCode" id="catCode" required class="form-control selectpicker-catcode with-ajax-catcode" data-live-search="true">
                                <option value="">Tap to search</option>
                                <?php foreach ($assetcat as $u) : ?>
                                    <option value="<?= $u['catCode']; ?>" data-subtext="<?= $u['assetCatName']; ?>"><?= $u['catCode']; ?> </option>
                                <?php endforeach; ?>
                            </select>
                        </template>
                    </form-inventorymaster>
                ...
                <div class="modal-footer">
                    ...
                    <a class="btn samrs-danger is-outline" href="<?= base_url('master_data/inventory_master'); ?>">Cancel</a>
                </div>
            </div>
        </form>
    </div>
                        ...
                        if (response.queryResult == true) {
                            if (!is_close) {
                                ...
                                Swal.fire({
                                    ...
                                    $('.samrs-table1').DataTable().ajax.reload();
                                    document.location.href = "<?php echo base_url(); ?>master_data/inventory_master";
                                });
