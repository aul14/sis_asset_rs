<main dir="ltr" class="page-wrapper full-height" id="App">
  <main-app>
    <action-button-card title="Master Data - Brands">
      <template v-slot:buttons>
        <table-init></table-init>
        <?php if ($result_role[4]['subMenu1'][2]['subMenu2'][0]['isAllow'] == true) : ?>
          <add-data modal="brand_form"></add-data>
        <?php endif; ?>
        <?php if ($result_role[4]['subMenu1'][2]['subMenu2'][1]['isAllow'] == true) : ?>
          <edit-data button-id="modal_edit_brand"></edit-data>
        <?php endif; ?>
        <?php if ($result_role[4]['subMenu1'][2]['subMenu2'][2]['isAllow'] == true) : ?>
          <delete-data button-id="modal_delete_brand"></delete-data>
        <?php endif; ?>
        <?php if ($result_role[4]['subMenu1'][2]['subMenu2'][3]['isAllow'] == true) : ?>
...
<script>
  function TableData() {
    let table = $('.samrs-table1').DataTable({
      ...
      "ajax": {
        "url": BASE_URL + "master_data/brand/data_table",
        "dataType": "json",
        "data": {},
        "type": "POST",
        "cache": true,
  ...
  function edit() {
    $("#modal_edit_brand").click(function() {
      // e.preventDefault();
      var idBrand = [];
      ...
      if (idBrand.length == 1) {
        $('#brand_form').modal('show');
        $.ajax({
          type: "POST",
          url: BASE_URL + "master_data/brand/brand_by_id",
          data: {
            'idBrand': idBrand
          },
          dataType: "json",
          success: function(res) {
            $("#title-brand").html("Update brand");
            $("input[name=formType]").val("edit");
            $("input[name=idBrand]").val(res.data_update.idBrand);
            $('input[name=brandName]').val(res.data_update.brandName);
          }
        });
        ...
        Swal.fire({
          icon: 'warning',
          title: 'Oops...',
          text: (idBrand.length > 1) ? "You choose more than 1 item, please choose one item to be edited" : "Select Items to be Edited",
        });
      }
    });
  }
  ...
  function hapus() {
    $("#modal_delete_brand").click(function() {
      // e.preventDefault();
      var idBrand = [];
      // Read all checked checkboxes
      $("input:checkbox[class=delete_check]:checked").each(function() {
          ...
          buttons: {
            hapus: {
              ...
              action: function() {
                $.ajax({
                  type: 'POST',
                  url: BASE_URL + "master_data/brand/delete",
                  data: {
                    'idBrand': idBrand,
