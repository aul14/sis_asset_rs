<main dir="ltr" class="page-wrapper full-height" id="App">
  <main-app>
    <action-button-card title="Files - Inspection Form">
      <template v-slot:buttons>
        <table-init></table-init>
        <?php if ($result_role[3]['subMenu1'][0]['subMenu2'][0]['subMenu3'][0]['isAllow'] == true) : ?>
          <add-data modal="inspection_form"></add-data>
        <?php endif; ?>
        <?php if ($result_role[3]['subMenu1'][0]['subMenu2'][0]['subMenu3'][1]['isAllow'] == true) : ?>
          <edit-data button-id="modal_edit_formins"></edit-data>
        <?php endif; ?>
        <?php if ($result_role[3]['subMenu1'][0]['subMenu2'][0]['subMenu3'][2]['isAllow'] == true) : ?>
          <delete-data button-id="modal_delete_formins"></delete-data>
        <?php endif; ?>
        <?php if ($result_role[3]['subMenu1'][0]['subMenu2'][0]['subMenu3'][3]['isAllow'] == true) : ?>
...
<script>
  function TableData() {
    let table = $('.samrs-table1').DataTable({
      ...
      "ajax": {
        "url": BASE_URL + "files/forms/inspection/data_table",
        "dataType": "json",
        "data": {
  ...
  function hapus() {
    $("#modal_delete_formins").click(function() {
      // e.preventDefault();
      var idFormTemplate = [];
  ...
  function edit() {
    $("#modal_edit_formins").click(function() {
      // e.preventDefault();
      var idFormTemplate = [];
      // Read all checked checkboxes
      $("input:checkbox[class=delete_check]:checked").each(function() {
      ...
      if (idFormTemplate.length == 1) {
        ...
        $.ajax({
          ...
          success: function(res) {
            ...
            $("#title-forms-main").html("update form inspection");
