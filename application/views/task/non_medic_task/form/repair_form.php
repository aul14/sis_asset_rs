    ...
    <form id="repair-form">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <a href="<?= base_url('task/non/repair'); ?>" class="btn btn-rounded btn-outline-danger" type="button" class="close" aria-hidden="true">×</a>
        </div>
        <div class="modal-body fixed-height samrs-form" id="app_form">
          <input type="hidden" name="taskSysCat" value="NON">
          <input name="taskCode" type="hidden" value="NCPL" />
          <form-repair>
        ...
        <div class="modal-footer">
          ...
          <a class="btn samrs-danger is-outline" href="<?= base_url('task/non/repair'); ?>">Cancel</a>
        </div>
      </div>
    </form>
  </div>
...
<script>
  $(function() {
    ...
    $('#repair-form').submit(function(e) {
      e.preventDefault();
      $.ajax({
        url: BASE_URL + 'task/non/repair/store',
        type: "POST",
          ...
          if (response.queryResult == true) {
            if (!is_close) {
              ...
              Swal.fire({
                ...
                $('.samrs-table1').DataTable().ajax.reload();
                document.location.href = "<?php echo base_url(); ?>task/non/repair";
              });
