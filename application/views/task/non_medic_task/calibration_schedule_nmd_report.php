<main dir="ltr" class="page-wrapper full-height" id="App">
  <main-app>
    ...
    <action-button-card title="Non Electromedic Task - Calibration Schedule & Report">
      <template v-slot:buttons>
        <table-init></table-init>
        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][0]['subMenu3'][0]['subMenu4'][0]['isAllow'] == true) : ?>
          <add-data modal="calibration_form"></add-data>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][0]['subMenu3'][0]['subMenu4'][1]['isAllow'] == true) : ?>
        ...
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][0]['subMenu3'][0]['subMenu4'][2]['isAllow'] == true) : ?>
          <delete-data button-id="modal_delete_calib"></delete-data>
        <?php endif; ?>

        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][0]['subMenu3'][0]['subMenu4'][3]['isAllow'] == true) : ?>
          <print-data modal="task_print"></print-data>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][0]['subMenu3'][0]['subMenu4'][4]['isAllow'] == true) : ?>
          <table-advance-search overlay="advanced_search"></table-advance-search>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][0]['subMenu3'][0]['subMenu4'][5]['isAllow'] == true) : ?>
...
<script>
  function TableData() {
    let table = $('.samrs-table1').DataTable({
      ...
      "ajax": {
        "url": BASE_URL + "task/non/calibration/schedule_report/data_table",
        "dataType": "json",
        "data": {
    ...
    $("#modal_edit_calib").click(function() {
      ...
      if (idTask.length == 1) {
        ...
        if (timeStart != "-") {
          ...
          $.ajax({
            type: "POST",
            url: BASE_URL + "task/non/calibration/schedule_report/by_id_calibration",
            data: {
              'idTask': taskReport,
              'idAsset': assetReport
            },
            ...
            success: function(res) {
              // console.log(res);
              let url_action_approve = BASE_URL + "task/non/calibration/schedule_report/update_action_approve";
              ...
              $.ajax({
                type: "POST",
                url: BASE_URL + "task/non/calibration/schedule_report/auto_date",
                data: {
          ...
          $.ajax({
            type: "POST",
            url: BASE_URL + "task/non/calibration/schedule_report/by_id_task",
            data: {
              'idTask': idTask
            },
            dataType: "json",
          ...
          buttons: {
            hapus: {
              ...
              action: function() {
                $.ajax({
                  type: 'POST',
                  url: BASE_URL + "task/non/calibration/schedule_report/delete",
                  data: {
                    'idTask': idTask,
                    'idAsset': idAssetDel,
