<main dir="ltr" class="page-wrapper full-height" id="App">
  <main-app>
    ...
    <action-button-card title="Non Electromedic Task - Insepection Schedule & Report">
      <template v-slot:buttons>
        <table-init></table-init>
        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][1]['subMenu3'][0]['subMenu4'][0]['isAllow'] == true) : ?>
          <approve-data button-id="modal_approve_inp"></approve-data>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][1]['subMenu3'][0]['subMenu4'][1]['isAllow'] == true) : ?>
          <add-data modal="inspection_form"></add-data>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][1]['subMenu3'][0]['subMenu4'][2]['isAllow'] == true) : ?>
          <div class="grid-box">
            <a href="javascript:void(0)" id="modal_edit_inp" class="btn btn-block samrs-success"><i class="fas fa-edit"></i> Edit</a>
          </div>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][1]['subMenu3'][0]['subMenu4'][3]['isAllow'] == true) : ?>
          <delete-data button-id="modal_delete_inp"></delete-data>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][1]['subMenu3'][0]['subMenu4'][8]['isAllow'] == true) : ?>
          <detail-data button-id="modal_detail_inp"></detail-data>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][1]['subMenu3'][0]['subMenu4'][4]['isAllow'] == true) : ?>
          <print-data modal="task_print"></print-data>
        <?php endif; ?>

        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][1]['subMenu3'][0]['subMenu4'][6]['isAllow'] == true) : ?>
          <table-advance-search overlay="advanced_search"></table-advance-search>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][1]['subMenu2'][1]['subMenu3'][0]['subMenu4'][7]['isAllow'] == true) : ?>
...
<script>
  function TableData() {
    let table = $('.samrs-table1').DataTable({
      ...
      "ajax": {
        "url": BASE_URL + "task/non/inspection/schedule_report/data_table",
        "dataType": "json",
        "data": {
    ...
    $("#modal_edit_inp").click(function() {
      ...
      if (idTask.length == 1) {
        ...
        if (timeStart != "") {
          ...
          let url_action_approve = BASE_URL + "task/non/inspection/schedule_report/store_form_old_edit";
          $("#app_form_edit").attr("action", url_action_approve);

          $.ajax({
            type: "POST",
            url: BASE_URL + "task/non/inspection/schedule_report/approve_id_task",
            data: {
              'idTask': idTask
            },
            dataType: "json",
          ...
          $.ajax({
            type: "POST",
            url: BASE_URL + "task/non/inspection/schedule_report/task_by_id",
            data: {
              'idTask': idTask
            },
            dataType: "json",
  ...
  function update_inp() {
    $(document).on('click', '#update-mtn', function() {
      ...
      $.ajax({
        type: "POST",
        url: BASE_URL + "task/non/inspection/schedule_report/update_inp",
        data: {
          'idTask': idTask,
          'idAsset': idAsset
        },
