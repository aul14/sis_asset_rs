<main dir="ltr" class="page-wrapper full-height" id="App">
  <main-app>
    ...
    <action-button-card title="Electromedic Task - Repair">
      <template v-slot:buttons>
        <table-init></table-init>
        <?php if ($result_role[2]['subMenu1'][0]['subMenu2'][5]['subMenu3'][0]['isAllow'] == true) : ?>
          <approve-data button-id="modal_approve_repair"></approve-data>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][0]['subMenu2'][5]['subMenu3'][1]['isAllow'] == true) : ?>
          <add-data modal="repair_form"></add-data>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][0]['subMenu2'][5]['subMenu3'][2]['isAllow'] == true) : ?>
          <edit-data button-id="modal_edit_repair" text="Finish/Pending"></edit-data>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][0]['subMenu2'][5]['subMenu3'][3]['isAllow'] == true) : ?>
          <delete-data button-id="modal_delete_repair"></delete-data>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][0]['subMenu2'][5]['subMenu3'][4]['isAllow'] == true) : ?>
          <detail-data button-id="modal_detail_repair"></detail-data>
        <?php endif; ?>

        <?php if ($result_role[2]['subMenu1'][0]['subMenu2'][5]['subMenu3'][5]['isAllow'] == true) : ?>
          <print-data modal="task_print"></print-data>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][0]['subMenu2'][5]['subMenu3'][6]['isAllow'] == true) : ?>
          <table-advance-search overlay="advanced_search"></table-advance-search>
        <?php endif; ?>
        <?php if ($result_role[2]['subMenu1'][0]['subMenu2'][5]['subMenu3'][7]['isAllow'] == true) : ?>
          <table-column-view></table-column-view>
        <?php endif; ?>
      </template>
      <template v-slot:content>
        <form action="" method="get">
          <advanced-search with-status="Yes" have-task-periode="yes" :radio-name="['status']" :radio-label="['Pending Work','Finish Not Approved','Approved']" :radio-value="['2','3','4']"></advanced-search>
        </form>
      </template>
    </action-button-card>
    <table-view-card>
...
<script>
    ...
    let table = $('.samrs-table1').DataTable({
      ...
      columns: [{
        ...
        },
        {
          title: 'Code Complain',
          name: 'idRelatedTask',
          data: "propTaskComplain.taskComplainCodeRepair",
          "defaultContent": "-"
        },
        {
          title: 'Status',
        ...
        },
        {
          ...
          name: 'repairNote',
          "data": "propTaskRepair.repairNote",
          "defaultContent": "-"
        },
        {
          title: 'Result',
          name: 'repairResult',
          "data": "propTaskRepair.repairResult",
          "defaultContent": "-"
        },
      ],
      ...
      if (idTask.length == 1) {
        $.ajax({
          ...
          success: function(res) {
            ...
            $('.name-teknisi').html(res.data_update.propProgress.finishBy);
  ...
  function hapus() {
    // create function delete
    $('#modal_delete_repair').click(function() {
      // e.preventDefault();
  ...
  function edit() {
    $("#modal_edit_repair").click(function() {
      // e.preventDefault();
      var idTask = [];
      // Read all checked checkboxes
      $("input:checkbox[class=delete_check]:checked").each(function() {
      ...
      if (idTask.length == 1) {
        ...
        $.ajax({
          ...
          dataType: "json",
          success: function(res) {
            $("#title-repair").html("Update Repair");
            ...
            $("input[name=idComplain]").val(res.data_update.idRelatedTask);
            $("input[name=complain]").val("CPL-" + res.data_update.idRelatedTask);
            ...
            if (res.data_update.idVendor != 0) {
              $('#option-ajax-vendor').html(res.data_update.propContact.contactCompany);
              $('#option-ajax-vendor').val(res.data_update.propContact.idContact);
              $('.selectpicker-supplier').selectpicker('refresh');
            }

            if (res.data_update.propProgress != null) {
              $('#assignTo').val(res.data_update.propProgress.assignTo);

              $('input[name=idAssignee]').val(res.data_update.propProgress.assignTo);
            }


            if (res.data_update.idVendor != 0) {
              $('#withTechVendor').prop('checked', true).change();
            } else {
              $('#withTechVendor').prop('checked', false).change();
            }

            if (res.data_update.propTaskRepair != null) {
              if (res.data_update.propTaskRepair.length > 0) {
                // for (i = 0; i < res.data_update.propTaskRepair.length; i++) {
                // $("input[type='radio'][name='repairResult']:checked");
                if (res.data_update.propTaskRepair[0].repairResult != "") {
                  $('.name-teknisi').html(res.data_update.propProgress.assignTo);
                  $("input[name=repairResult][value='" + res.data_update.propTaskRepair[0].repairResult + "']").prop('checked', true);
                  $("textarea[name=repairNote]").val(res.data_update.propTaskRepair[0].repairNote);
                }

                // console.log(res.data_update.propTaskRepair[0].idForm);

                if (res.data_update.propTaskRepair[0].propForm.propFormPic.length > 0) {
                  $('#withTechAssistant').prop('checked', true).change();
                } else {
                  $('#withTechAssistant').prop('checked', false).change();
                }

                if (res.data_update.propTaskRepair[0].propForm.propFormTools.length > 0) {
                  $('#withActionTools').prop('checked', true).change();
                } else {
                  $('#withActionTools').prop('checked', false).change();
                }

                if (res.data_update.propTaskRepair[0].propForm != null) {
                  $("input[name=idForm]").val(res.data_update.propTaskRepair[0].idForm);
                  $('.techassistant_list').DataTable().ajax.url(BASE_URL + "task/med/task_datatable/list_data_pic/" + res.data_update.propTaskRepair[0].idForm).load();
                  $('.progressrecord_list').DataTable().ajax.url(BASE_URL + "task/med/task_datatable/list_data_record/" + res.data_update.propTaskRepair[0].idForm).load();
                  $('.sparepartusage_list').DataTable().ajax.url(BASE_URL + "task/med/task_datatable/list_data_stock/" + res.data_update.propTaskRepair[0].idForm).load();
                  $('.servicecharge_list').DataTable().ajax.url(BASE_URL + "task/med/task_datatable/list_data_service/" + res.data_update.propTaskRepair[0].idForm).load();
                  $('.toolsusage_list').DataTable().ajax.url(BASE_URL + "task/med/task_datatable/list_data_tools/" + res.data_update.propTaskRepair[0].idForm).load();
                }

                if (res.data_update.propTaskRepair[0].isNeedPart == true) {
                  $('#withActionSparepart').prop('checked', true).change();
                } else {
                  $('#withActionSparepart').prop('checked', false).change();
                }

                if (res.data_update.propTaskRepair[0].isPending == false) {
                  $('#isAlreadyFinish').prop('checked', true).change();
                } else {
                  $('#isAlreadyFinish').prop('checked', false).change();
                }

                if (res.data_update.propTaskRepair[0].propForm.propFormServices.length > 0) {
                  $('#withActionService').prop('checked', true).change();
                } else {
                  $('#withActionService').prop('checked', false).change();
                }
              ...
              if (res.data_update.propTaskFiles.length > 0) {
                ...
                for (i = 0; i < res.data_update.propTaskFiles.length; i++) {
                  ...
                  <div class="card" style="width: 8rem;">
                    <img src="data:image/png;base64,` + res.view_task_file[i] + `" class="card-img-top" alt="...">
                    <div class="card-body">
        ...
        Swal.fire({
          icon: 'warning',
          title: 'Oops...',
          text: (idTask.length > 1) ? "You choose more than 1 item, please choose one item to be Finish/Pending" : "Select Items to be Finish/Pending",
        });
      }
